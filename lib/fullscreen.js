...
function reducer(state, action) {
    switch (action.type) {
        case ImageGroup.actions.toggleExpand:
            return __assign(__assign({}, state), { currentFocusedImageIndex: action.payload.id, isImageGroupExpanded: true, shouldAnimate: false });
        case ImageGroup.actions.toggleExpandAnimate:
            return __assign(__assign({}, state), { currentFocusedImageIndex: action.payload.id, isImageGroupExpanded: true, shouldAnimate: true });
        case ImageGroup.actions.toggleCloseAnimate:
            return __assign(__assign({}, state), { currentFocusedImageIndex: -1, isImageGroupExpanded: false, shouldAnimate: true });
        default:
...
function calculatePosition(action) {
    return function calculate(el, transitionMs) {
        ...
        if (action === 'open') {
            ...
            var widthScale = innerWidth_1 / width;
            var heightScale = innerHeight_1 / height;
            var scaleBy = widthScale < heightScale ? 'width' : 'height';
            var scale = scaleBy === 'width' ? widthScale : heightScale;
